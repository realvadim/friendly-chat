    ...
    func fetchConfig() {
        ...
        remoteConfig.fetch(withExpirationDuration: expirationDuration) { (status, error) in
            if status == .success {
                ...
                //replaces current cached conf value
                self.remoteConfig.activateFetched()
                let friendlyMsgLength = self.remoteConfig["friendly_msg_length"]
                //ensure it came from remote config instead 
                //of default variable in the app
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let imageUrl = message[Constants.MessageFields.imageUrl] {
            //If the image is already cached, it doesn’t need to be downloaded again
